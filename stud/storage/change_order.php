...
<?php
  ...
  if (isset($_POST['save']))
  {
      ...
      if ($idd == "")
      {
          echo "<b> Вы не ввели номер заказа, попробуйте снова<b></br><a href = 'unpayd.php'> Главная </a>";
          return;
      }
  ...
  function RememberData_save($cur_order)
    {
        include_once "help_func.php";
        ...
        for($i = 0; $i < $count; $i++)
        {
            if ($mas[$i] == 'адрес')
            {        
                      $_SESSION[$mas[$i]] = $cur_order['адрес_доставки'];
                      continue;
            }

            if (empty($cur_order[$mas[$i]]))
              continue;

              var_dump($cur_order[$mas[$i]]);

            $_SESSION[$mas[$i]] = $cur_order[$mas[$i]];
        }
        $_SESSION['сумма'] = $cur_order['сумма'];
    }
?>
